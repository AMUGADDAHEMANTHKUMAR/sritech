...
export type SelectedPath = 'beginner' | 'professional';
type CourseSelection = 'student' | 'workexperience';
...
export const workExperienceProjects: Project[] = [
  { 
    id: 101, 
    title: "RAP ABAP", 
    cat: "Course", 
...
export const allProjects: Project[] = [...studentProjects, ...workExperienceProjects];

const selectedPathToSelection: Record<SelectedPath, CourseSelection> = {
  beginner: 'student',
  professional: 'workexperience'
};

const selectionToSelectedPath: Record<CourseSelection, SelectedPath> = {
  student: 'beginner',
  workexperience: 'professional'
};

function getReturnedPath(state: unknown): SelectedPath | null {
...
const ProjectCard: React.FC<{ project: Project; selectedPath: SelectedPath }> = ({ project, selectedPath }) => {
  const navigate = useNavigate();

  const handleEnroll = () => {
    navigate(`/enroll/${encodeURIComponent(project.title)}`, {
      state: { selectedPath }
    });
  };
...
export default function WorkGallery() {
  ...
  const [selection, setSelection] = useState<CourseSelection | null>(() => {
    const returnedPath = getReturnedPath(location.state);
    return returnedPath ? selectedPathToSelection[returnedPath] : null;
  });
  ...
  const selectedPath = selection ? selectionToSelectedPath[selection] : 'beginner';
  ...
  useEffect(() => {
    ...
    setSelection(selectedPathToSelection[returnedPath]);
      ...
      <div className="container">
        ...
        {selection && (
          <>
            ...
            {/* Courses Grid */}
            <div ref={coursesContainerRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 px-4 md:px-8 lg:px-12">
              {projects.map((project) => (
                <ProjectCard key={project.id} project={project} selectedPath={selectedPath} />
              ))}
            </div>
          </>
        )}
